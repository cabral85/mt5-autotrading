...
class Setup90:
    ...
    def __update_bars(self):
        self.__stock_bars = self.__bars.get_stock_bars(self.__stock_name, DEFAULT_N_BARS, self.mt5.TIMEFRAME_M5)
    ...
    def __update_indicators(self):
        ...
        self.__long_sma: list = simple_moving_average(self.__close_prices, LONG_SMA_VALUE)
    ...
    def __check_for_trade(self):
        ...
        if self.__short_sma[-1] > self.__long_sma[-1] and self.__short_sma[-5] > self.__long_sma[-5] and trend:
            if self.__low_prices[-1] <= self.__short_sma[-1] < self.__close_prices[-1]:
                self.__msg.send_notification(f"{SETUP_NAME} - Buy", self.__stock_name)
        if self.__short_sma[-1] < self.__long_sma[-1] and self.__short_sma[-5] < self.__long_sma[-5] and trend:
            if self.__high_prices[-1] >= self.__short_sma[-1] > self.__close_prices[-1]:
